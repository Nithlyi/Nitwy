...
class PingSelect(Select):
    def __init__(self, cog, roles_dict, categoria, placeholder="Escolha uma opção..."):
        ...
        super().__init__(placeholder=placeholder, options=options, min_values=1, max_values=1, custom_id=f"ping:{categoria}")
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Atribui o novo role (sem remover antigos)
        await interaction.user.add_roles(role)
        
        # Mensagem de confirmação
        await interaction.response.send_message(f"'{opcao_nome}' atribuído a você!", ephemeral=True)

class PainelPing(View):
    def __init__(self, cog):
        super().__init__(timeout=None)
        self.add_item(PingSelect(cog, cog.roles_pings, "pings", "Escolha um ping..."))
...
class Ping(commands.Cog):
    def __init__(self, bot):
        ...
        self.bot.add_view(PainelPing(self))
    ...
    def load_config(self):
        if self.collection is not None:
            ...
            if doc:
                ...
                self.embed_description = doc.get("embed_description", "Escolha um ping abaixo:")
    ...
    def set_defaults(self):
        ...
        self.embed_description = "Escolha um ping abaixo:"
    ...
    roles_pings = {
        "Reviver Chat": 0x00B894,
        "Anúnciamentos": 0xE17055,
        "Parcerias": 0x6C5CE7,
        "Ir Call": 0xFD79A8
    }
    ...
    class EditModal(Modal):
        ...
        async def on_submit(self, interaction: discord.Interaction):
            ...
            try:
                ...
                await interaction.response.send_message("A interação expirou (possivelmente após reinicialização do bot). Reabra o painel com /config_ping.", ephemeral=True)
    ...
    # Slash command para configurar o ping (apenas admins, ephemeral)
    @app_commands.command(name="config_ping", description="Abre o painel de configuração do ping com preview (ephemeral)")
    ...
    # Slash command para abrir o painel de ping (persistente, não ephemeral)
    @app_commands.command(name="painel_ping", description="Abre o painel de seleção de ping no canal atual")
    async def painel_ping(self, interaction: discord.Interaction):
        embed = self.create_preview_embed()
        view = PainelPing(self)
        await interaction.channel.send(embed=embed, view=view)  # Envia no canal, persistente
        await interaction.response.send_message("Painel de ping enviado no canal!", ephemeral=True)  # Confirmação privada
